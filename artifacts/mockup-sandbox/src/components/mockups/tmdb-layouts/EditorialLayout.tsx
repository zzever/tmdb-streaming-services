    ...
    <div className="min-h-screen bg-[#0c0c13] text-white font-sans">
      <header className="sticky top-0 z-30 bg-[#0c0c13]/90 backdrop-blur-xl border-b border-white/8">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center gap-4">
          ...
          <div className="ml-auto flex items-center gap-3">
            <div className="flex items-center gap-1.5 text-xs text-white/40">
              ...
              <select value={locale} onChange={e => setLocale(Number(e.target.value))} className="bg-transparent focus:outline-none cursor-pointer text-white/50">
                {LOCALES.map((l, i) => <option key={l} value={i}>{l}</option>)}
              </select>
            </div>
            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/15 border border-amber-500/25 rounded-lg text-xs font-semibold text-amber-400">
              <ExternalLink className="w-3 h-3" /> Stremio
            </button>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6 flex gap-0">
          {[{ id: "movie", label: "Movies", icon: Film }, { id: "series", label: "Series", icon: Tv }].map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setActiveType(id as "movie" | "series")}
              className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold border-b-2 transition-all ${
                activeType === id ? "border-amber-400 text-amber-400" : "border-transparent text-white/35 hover:text-white/60 hover:border-white/20"
              }`}
            >
      ...
      <main className="max-w-6xl mx-auto px-6 py-7">
        <div className="flex items-center justify-between mb-5">
          ...
          <button className="text-xs text-amber-400/70 hover:text-amber-400 flex items-center gap-1">
        ...
        <div className="grid grid-cols-3 gap-5">
          ...
          <div className="col-span-1 group cursor-pointer">
            <div className="relative rounded-2xl overflow-hidden aspect-[2/3] bg-white/5 shadow-2xl shadow-black/60">
              <img src={FEATURED.poster} alt={FEATURED.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <span className="text-[10px] font-bold uppercase tracking-widest text-amber-400 bg-amber-400/15 border border-amber-400/25 px-2 py-0.5 rounded-full mb-2 inline-block">
                  Featured
                </span>
                <h2 className="text-2xl font-bold text-white leading-tight mb-2">{FEATURED.title}</h2>
                <p className="text-xs text-white/50 leading-relaxed line-clamp-2 mb-3">{FEATURED.overview}</p>
                <div className="flex items-center gap-1 mb-4">
                  <Star className="w-3.5 h-3.5 text-yellow-400 fill-yellow-400" />
                  <span className="text-sm font-bold text-yellow-400">{FEATURED.rating}</span>
                  <span className="text-xs text-white/30 ml-1">· {FEATURED.year}</span>
                </div>
                {/* Provider logos */}
                <div className="flex gap-2">
                  {FEATURED.providers.map((p, i) => (
                    <div key={i} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-white/10 backdrop-blur-md border border-white/15 hover:bg-white/20 transition-colors cursor-pointer">
                      {LOGO[p.name] && (
                      ...
                      <span className={`text-[10px] ${p.type === "flatrate" ? "text-emerald-400" : "text-amber-400"}`}>
                        {p.type === "flatrate" ? "Streaming" : "Alquiler"}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <div className="col-span-2 grid grid-cols-3 gap-4">
            {GRID.map((item) => (
              <div key={item.id} className="group cursor-pointer">
                ...
                {/* Provider logos below title */}
                <div className="flex items-center gap-1.5 mt-1.5">
                  {item.providers.map((p, i) =>
                    LOGO[p.name] ? (
                      <img key={i} src={LOGO[p.name]} alt={p.name} title={p.name}
                        className="w-5 h-5 rounded object-cover border border-white/10" />
                    ) : (
                      <span key={i} className="text-[10px] text-white/40 bg-white/6 px-1.5 py-0.5 rounded">{p.name}</span>
                    )
                  )}
                  <span className="text-[10px] text-white/25 ml-0.5">{item.year}</span>
                </div>
              </div>
            ))}
          </div>
